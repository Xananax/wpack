 ...
 * 
 * This shortcut creates a web production build. Any flag pertaining to type of build in the passed env or in the options file are overriden.
 * 
 ...
 */
export default function getWebConfigProduction(env:WPACK.ENV, optionsFilename:string, returnWebpackConfig:boolean):WEBPACK.CONFIG;
export default function getWebConfigProduction(env:WPACK.ENV, optionsFilename:string):WPACK.CONFIG;
export default function getWebConfigProduction
	...
	{
		return returnWebpackConfig ? getClientConfig(env,optionsFilename,true,true) : getClientConfig(env,optionsFilename,true);
	}