 ...
 * 
 * This shortcut creates a server development build. Any flag pertaining to type of build in the passed env or in the options file are overriden.
 * 
 ...
 */
export default function getServerConfigDevelopment(env:WPACK.ENV, optionsFilename:string, returnWebpackConfig:boolean):WEBPACK.CONFIG;
export default function getServerConfigDevelopment(env:WPACK.ENV, optionsFilename:string):WPACK.CONFIG;
export default function getServerConfigDevelopment
	( env:WPACK.ENV
	, optionsFilename:string
	, returnWebpackConfig:boolean=false
	):WPACK.CONFIG | WEBPACK.CONFIG
	{
		return returnWebpackConfig? getServerConfig(env,optionsFilename,false,true) : getServerConfig(env,optionsFilename,false);
	}